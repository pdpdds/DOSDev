...
void xscrollup_han(int x1, int y1, int x2, int y2)
/* scrolls up one line, more usual routine than scroll down */
{
   unsigned scr_width;                  /* equivalent to one register */

   asm  push    es

   /* scroll up screen buffer */

   asm  les     di, dword ptr scr_buff  /* ES = destination segment */
   asm  mov     ax, y1                  /* DI = destination offset  */
   ...
   asm  mov     si, di
   asm  mov     ax, LIMITX_HAN          /* SI = source offset */
   asm  shl     ax, 1
   asm  shl     ax, 1
   asm  add     si, ax
   ...
   /* erase the last line of the screen buffer */
   ...
   asm  cmp     cx, 0
   asm  jbe     exit_erase_last_scr_buff
   asm  stosw
   asm  push    ax
   asm  mov     ax, bx
   asm  stosw
   asm  dec     cx
   asm  pop     ax
   asm  jmp     erase_last_scr_buff

   exit_erase_last_scr_buff:

   /* scroll up visual screen */
   ...
   asm  mov     si, abs_maxx_han        /* SI = source offset */
   asm  inc     si
   asm  shl     si, 1
   asm  shl     si, 1
   asm  add     si, di
   ...
   asm  pop     di
   asm  pop     si
   asm  add     si, scr_width
   asm  add     di, scr_width
   asm  dec     ax
   ...
   /* erase the last text line in HGC */
   ...
   asm  jmp     quit_xscrollup_han

   vga_scroll:

   /* scroll up in EGA/VGA */
   ...
   asm  mov     ax, VGA_SCR_WIDTH       /* SI = source offset */
   asm  inc     cx
   asm  mul     cx
   asm  shl     ax, 1
   asm  shl     ax, 1
