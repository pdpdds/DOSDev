...
const char *TextDirect[] = {
  "HorizDir",  "VertDir"
};

const char *HorizJust[] = {
...
//  ReportStatus();		/* Report results of the initialization */
  ...
  if( GraphDriver==EGA || GraphDriver==EGALO || GraphDriver==VGA ||
      GraphDriver==134 )        /* allow color demo with installed driver */
    PaletteDemo();
...
//  int styles[]	  = {    1, 3, 10, 5, 9, 1	};
  ...
  settextjustify( CENTER_TEXT, TOP_TEXT );
  for( i=0 ; i<6 ; ++i ){
    setfillstyle(1, random(MaxColors) );
...
void PaletteDemo(void)
{
  int i, j, x, y, color;
  struct viewporttype vp;
  int height, width, Pixcell;

  MainWindow( "Palette Demonstration" );
  StatusLine( "Press any key to continue, ESC to Abort" );

  if (getmaxcolor() == 255) Pixcell = 16; else Pixcell = 4;
  getviewsettings( &vp );
  width  = (vp.right - vp.left) / Pixcell;	/* get width of the box 	*/
  height = (vp.bottom - vp.top) / Pixcell;	/* Get the height of the box	*/

  x = y = 0;				/* Start in upper corner	*/
  color = 0;				/* Begin at 1st color		*/
  BuildColors();
  for( j=1 ; j<Pixcell+1; ++j ){	/* For 10 rows of boxes 	*/
    for( i=1 ; i<Pixcell+1; ++i ){	/* For 15 columns of boxes	*/
  ...
  while( !kbhit() ){			/* Until user enters a key...	*/
    if (getmaxcolor()==255) setrgbpalette(random(254)+1,random(254)+1,
					  random(254)+1,random(254)+1);
  }
