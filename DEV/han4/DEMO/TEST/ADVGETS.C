...
/* adv_gets_han() event function pointer */
void (*adv_gets_han_event_func) (char *full_str,
				 char *each_str,
				 union hangul *word);
...
/* WordStar-type string input function */
{
   ...
   do
   {
	 ...
	 default            :
	    ...
	    if(adv_gets_han_event_func != NULL)
	       adv_gets_han_event_func(temp, han_str, &cur_han);
	    return(return_code);
      }
