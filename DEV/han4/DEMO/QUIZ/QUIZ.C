...
/* Æá»a ÑÁ·©·i ·ª´á —i·¥”a. µAœáˆa ¤i¬—Ða¡e 0·i Ÿ¡ÈåÐe”a. */
{
   ...
   /* ÑÁ·©·¡ ·¶·a¡e ·ª´á¬á quizµA ¸á¸wÐe”a. */
   {
         ...
         !strcmp(header, FILE_HEADER))
      {
         ...
         {
	    quiz = (quiz_struct *) farmalloc(f_length);
            if(quiz == NULL)
            {
...
/* mode = 0 ·¡¡e Âˆa ¬wÈ, 1 ·¡¡e ®¸÷/¬b¹A ¬wÈˆa –E”a. */
{
   ...
   {
      quiz = (quiz_struct *) farmalloc(MAX_APPEND*sizeof(quiz_struct));
      if(quiz == NULL) return;
...
/*                   ¡A¡¡Ÿ¡ˆa ¦¹¢Ði ˜                      */
{
   ...
   if((made     = farmalloc(total_num))              == NULL ||
      (quiz_ptr = farmalloc(sizeof(quiz)*total_num)) == NULL)
   {
      put_msg("¡A¡¡Ÿ¡ˆa ¦¹¢Ðs“¡”a. ´a¢ Ç¡a ’Ÿa­A¶a.", -1);
      if(made     != NULL) farfree(made);
      if(quiz_ptr != NULL) farfree(quiz_ptr);
      return ESC;
   }
   /* made “e ¡¡– 0·a¡ Á¡‹¡ÑÁÐe”a. */
...
/* ”aŸe Æá»a ÑÁ·©·i ·ª´á¬á Ñe¸ ¬åÈ‚–E ·AÑwµA ÂˆaÐe”a. */
{
   ...
   if(f_length != 0)
   {
      ...
      if((quiz = (quiz_struct *) farmalloc(f_length)) == NULL)
      {
         put_msg("¡A¡¡Ÿ¡ˆa ¦¹¢Ðs“¡”a. ´a¢ Ç¡a ’Ÿa­A¶a.", -1);
         result = 0;
      }
