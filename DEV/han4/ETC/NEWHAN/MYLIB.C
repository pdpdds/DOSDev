...
/* ¯aµÁ §¡ŠaÐ ¥© ˆõ.                                                     */
{
   ...
   /* ÑÁ¡e·i ¸á¸wÐe”a. */
   size = imagesize_han(0, 0, abs_maxx_han+1, 2);
   if((screen = malloc(size)) == NULL)
   {
   ...
   getimage_han(0, 0, abs_maxx_han+1, 2, screen);
...
/* ­¡¯aµÁ §¡ŠaÐ ¥© ˆõ.                                                  */
{
   ...
   /* ÑÁ¡e·i ¸á¸wÐe”a. */
   size = imagesize_han(box_x1, box_y1, box_x2+1, box_y2+1);
   if((screen = malloc(size)) == NULL) return -2;
   getimage_han(box_x1, box_y1, box_x2+1, box_y2+1, screen);
   ...
   /* ÑÁ¡e·i ¥¢ŠÐe”a. */
   putimage_han(box_x1, box_y1, screen);
   free(screen);
...
/* ’‰a»¥ Ç¡µA ˜aœa YES, NO, ESCŸi Ÿ¡ÈåÐe”a.                               */
{
   ...
   /* ÑÁ¡e·i ¸á¸wÐe”a. */
   size = imagesize_han(box_x, y, box_x+strlen(string)+2, y+5);
   if((screen = malloc(size)) == NULL) return(ESC);
   getimage_han(box_x, y, box_x+strlen(string)+2, y+5, screen);
...
/* wait_time·¡ -1·¡¡e ‰­¢ Ç¡ ·³b·i ‹¡”aŸ¥”a.                        */
{
   ...
   /* ÑÁ¡e·i ¸á¸wÐe”a. */
   size = imagesize_han(box_x, 10, box_x+strlen(string)+2, 13);
   if((screen = malloc(size)) == NULL) return;
   getimage_han(box_x, 10, box_x+strlen(string)+2, 13, screen);
